...
const OutputView = {
  ...
  printPriceBeforeEvent(menus) {
    ...
    Console.print(this.price.toLocaleString() + "원\n");
  },
  printGetFree() {
    Console.print("<증정 메뉴>");
  ...
  printTotalEvent(date, menu) {
    ...
    if (EventCheck.isDday(date)) {
      Console.print(
        `크리스마스 디데이 할인: -${EventCheck.isDday(date).toLocaleString()}원`
      );
      this.sale += EventCheck.isDday(date);
    }
    if (EventCheck.isWeek(date, menu)) {
      Console.print(
        `평일 할인: -${EventCheck.isWeek(date, menu).toLocaleString()}원`
      );
      this.sale += EventCheck.isWeek(date, menu);
    }
    if (EventCheck.isWeekend(date, menu)) {
      Console.print(
        `주말 할인: -${EventCheck.isWeekend(date, menu).toLocaleString()}원`
      );
      this.sale += EventCheck.isWeekend(date, menu);
    }
    if (EventCheck.isStar(date)) {
      Console.print(
        `특별 할인: -${EventCheck.isStar(date).toLocaleString()}원`
      );
  ...
  printTotalSale() {
    ...
    if (this.sale === 0) {
      Console.print(`${this.sale.toLocaleString()}원\n`);
    }
    if (this.sale !== 0) {
      Console.print(`-${this.sale.toLocaleString()}원\n`);
    }
